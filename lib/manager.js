...
Manager.prototype.handleClient = function (data, req) {
  ...
  if (!~this.get('transports').indexOf(data.transport)) {
    /**
     * check if this connection is from socket.io 0.6
     */
    if (data.path === '/websocket' && this.verifyOrigin(req)) {
      this.log.debug('serving socket.io 0.6 client!');

      var id = self.generateId();
      data.protocol = 1;
      data.transport = 'websocket-06';
      data.id = id;
      data.path = '/1/websocket/'+id;

      var handshakeData = this.handshakeData(data);
      self.onHandshake(id, handshakeData);
      self.store.publish('handshake', id, handshakeData);
    } else {
