...
function WebSocket (mng, data, req) {
  ...
  var self = this;
  /**
   * needs to have its own heartbeat setting to serve socket.io 0.6 client.
   */
  this.heartbeats = 0;
  this.heartbeat_timeout = 8;
  this.heartbeat_interval = 10;
...
WebSocket.prototype.onSocketConnect = function () {
  ...
  if (waitingForNonce) {
    ...
    /**
     * socket.io 0.6 client needs session id after the handshake.
     */
    ...
    if (packet.type !== undefined) {
      switch (packet.type) {
        ...
        case 'json':
          ...
          break;
        default:
          this.log.debug('non-supported packet');
          this.log.debug(data);
          return;
      }
...
WebSocket.prototype.onMessage = function (data) {
  ...
  if (messages === false) {
    this.log.debug('Bad message received from client ' + this.id);
    return;
  }
